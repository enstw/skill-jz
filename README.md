...
- **[context-cleanup](./context-cleanup/)** — Clean and correct accumulated session logs in the current project, preserve current rules and unique history, route each kind of information to its proper home, and repair handoff instructions to prevent recurrence. Uses the project’s existing files and tools; no workflow-engine dependency.
...
- **[robust-web-fetch](./robust-web-fetch/)** — Retrieve known URLs through TLS impersonation, archives, and anti-detect browsing; validate before atomic writes, report JSON provenance, and explicitly opt into web-page printing.
- **[authenticated-fetch](./authenticated-fetch/)** — User-assisted EZproxy and institutional login (or a single-submit `login` from a credential file the user stored), persistent visible browser sessions, PDF discovery/download, and complete-or-explicitly-partial chapter merging. Depends on browser-cdp and robust-web-fetch.
...
- **[long-run](./long-run/)** — Work autonomously through a project's work queue until it is done or genuinely blocked. Defines the only legitimate stop reasons (`queue-empty` / `needs-human` / `needs-authorization` / `user-request`), blocked-means-skip, checkpoint-not-pause, and choice points settled without asking. Ships two Claude Code hooks (installed by init-machine): one arms a `.long-run` flag when your prompt says "long run" / "長跑", the other refuses an end-of-turn lacking a valid `LONG-RUN STOP:` line while the flag is armed (with idle, cost and staleness brakes). Slash-invocable as `/long-run`.
- **[jev-decide](./jev-decide/)** — Settle a 2–4-option decision with TypeSafe's Jev as a gated advisor: spec-authoring discipline (rich state, pros AND cons, docs-first) plus a margin / confidence / irreversible gate that returns PROCEED or ESCALATE. Advisory normally; inside a long run PROCEED means act and ESCALATE means park. Needs a TypeSafe API key.
- **[init-machine](./init-machine/)** — Set up Claude Code's global environment on a new machine (macOS or Ubuntu) from the canonical copies bundled in the skill: lease-based keep-awake hooks (the machine stays awake while the agent works, then the inhibition self-expires), cross-platform notification chimes, and the long-run stop guard. Merges into `~/.claude` without clobbering existing settings. The one deliberately runner-specific skill in this collection — its subject is Claude Code's own config. Slash-invocable as `/init-machine`.
- **[send-file](./send-file/)** — Get a file from this machine onto the user's other device (phone, laptop) or to another person, from a box the recipient can't reach directly, by streaming it through the hosted [piping-server](https://github.com/nwtgck/piping-server) relay at ppng.io — no account, nothing stored, sender→receiver with backpressure, NAT-friendly. Triggers on "send me the file" / "I want this on my phone" / "pass this to …" / "傳給我", and whenever the agent has just produced a file the user needs off this shell. Preempts spinning up `python -m http.server`, uploading to transfer.sh/a pastebin, or committing a file just to move it. The relay path is the only secret (generate a long random one), single-use/one-receiver, and the relay sees plaintext — so encrypt sensitive content first (age/gpg/7z, key out-of-band) or use piping-ui.org for browser E2E. Always hands the user two outputs — an ASCII QR of the URL (`qrencode -t UTF8`, or `uvx --from qrcode qr --ascii` where qrencode is absent) for a phone to scan, and a clickable URL for a browser (no receiving `curl`: a shell that can reach the machine just `cp`/`scp`s). Folds in the macOS Quick Look (`qlmanage`) local-viewer path for the person at this machine. Migrated from the machine's global preferences. Description-triggered + `/send-file`. *(Renamed from `ppng-handoff`.)*
- **[tui-drive](./tui-drive/)** — Drive an interactive terminal program you can't type into — a TUI, a slash command that exists only at an agent CLI's interactive prompt (e.g. Claude Code's `/artifacts` to list/copy/rename/pin/delete published artifacts), a wizard or login prompt, a REPL — when running headlessly with no tool or flag for it: launch the program in a detached `tmux` session and drive it with literal `send-keys` + `capture-pane` read-back (Escape-then-Enter to beat autocomplete; capture between keystrokes because they land intermittently; confirm the target before any destructive key; `kill-session` on every exit). Triggers on "delete that artifact" / "run /artifacts for me" / "answer the prompts", on "not a tty" errors, and at the moment the agent is about to say "you'll have to do that yourself in the app". Strictly for pre-authorized actions that merely lack a headless path — never to route around a refusal, a confirmation the user hasn't given, or a capability absent by design, and never as cover for reverse-engineering a service's endpoints or lifting its credentials. Description-triggered + `/tui-drive`. *(Renamed from `slash-command-autonomous`.)*
