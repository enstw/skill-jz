...
- **[genimage-canvas](./genimage-canvas/)** — Draw a single designed image (poster, banner, slide) by having `render.sh` invoke Claude Code's stock canvas-design skill to author an HTML composition, then rasterizing it to PNG via browser-screenshot. Exact on-image text (CJK-safe). Same contract as genimage-img2/genimage-nb. Slash-invocable as `/genimage-canvas`. **Depends on:** `claude` authenticated with stock `canvas-design`, plus browser-screenshot.
