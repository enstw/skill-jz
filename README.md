...
- **[long-run](./long-run/)** — Work autonomously through a project's work queue until it is done or genuinely blocked. Defines the only legitimate stop reasons (`queue-empty` / `needs-human` / `needs-authorization` / `user-request`), blocked-means-skip, checkpoint-not-pause, and choice points settled without asking. Ships a Claude Code `Stop` hook (installed by init-machine) that refuses an end-of-turn lacking a valid `LONG-RUN STOP:` line while a `.long-run` flag is armed. Slash-invocable as `/long-run`.
