...
- **[browser-cdp](./browser-cdp/)** — The one browser foundation: **see** a rendered page (screenshot / DOM dump via the gstack browse daemon when installed, else the hardened `scripts/shot.sh` CDP pipeline), **drive** a page programmatically (copy-in templates: `find-browser.mjs` discovery — `BROWSER_BIN` → provisioned wrapper → Brave/Chrome/chromium → playwright shell — and `cdp-client.mjs`: `launch` → `evalJs`/`send`/`close`), and **provision** a user-space headless Chromium where none exists (`scripts/provision.sh`: no root, no snap, container-safe; extracts missing libs via `apt download` + `dpkg -x`, probes the sandbox, emits a self-contained `BROWSER_BIN` wrapper). Zero npm dependencies, minimal env footprint. Absorbs the former **browser-screenshot** and **headless-chromium** skills. Description-triggered + `/browser-cdp`.
