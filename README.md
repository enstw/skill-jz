...
- **[img2](./img2/)** — Generate a single image with OpenAI gpt-image-2 via the Codex CLI. Bundles `gen-image.sh` with the shared renderer contract (`IMAGE_OK <path>` / `IMAGE_FAIL <reason>`) that image-deck workflows loop over. Slash-invocable as `/img2`. **Pre-condition:** `codex` installed and authenticated.
- **[img-nb](./img-nb/)** — Generate a single image with nano banana (Gemini's image model) via the agy (Antigravity) CLI. `gen-image.sh` is drop-in interchangeable with img2's (same contract) and encodes agy's quirks: scratch-dir cwd, JPEG artifacts converted to PNG, and safe stdout handling. Slash-invocable as `/img-nb`. **Pre-condition:** `agy` installed and logged in.
- **[img-canvas](./img-canvas/)** — Draw a single designed image (poster, banner, slide) by authoring an HTML composition following the canvas-design method, rasterized to PNG by `render.sh` via browser-screenshot. Exact on-image text (CJK-safe), no external CLI, no cost. Same contract as img2/img-nb. Slash-invocable as `/img-canvas`. **Depends on:** the canvas-design and browser-screenshot skills (pre-flight warns/stops if missing).
