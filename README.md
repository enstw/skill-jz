...
- **[browser-e2e](./browser-e2e/)** — Give any web app a real-browser e2e test suite with **zero test dependencies**: plain node scripts (node ≥22) driving a headless Chromium over CDP. Bundles copy-in templates — browser discovery (`BROWSER_BIN` → Brave/Chrome/chromium → playwright shell), the shared CDP client (`launch` → `evalJs`/`send`/`close`), and an example suite — plus the suite taxonomy, the JSON verdict contract, and the dead-server rule for service-worker offline testing (network emulation never reaches SWs). Includes an honest "when to use playwright instead" decision rule. Description-triggered + `/browser-e2e`.
- **[bookworm-e2e](./bookworm-e2e/)** — Project overlay for the e2e suites of the [bookworm](https://github.com/enstw/bookworm) reading server: the suite matrix, the two-stage offline runbook, and bookworm-specific conventions (中文-first string asserts, phone-shaped 直排 windows). Defers all method to **browser-e2e** — the exemplar of that skill's per-project-runbook pattern. Slash-invocable as `/bookworm-e2e`.
