...
- **[bookworm-e2e](./bookworm-e2e/)** — Run or author the e2e suites of the [bookworm](https://github.com/enstw/bookworm) reading server: the suite matrix (pure-node / self-contained / dev-server-backed), the two-stage offline runbook that needs a genuinely dead server (DevTools offline emulation doesn't reach service workers), browser discovery, and the conventions a new suite must follow. Project-specific by design — the repo's `CLAUDE.md` points here. Slash-invocable as `/bookworm-e2e`.
