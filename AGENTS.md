...
- `genimage-canvas/SKILL.md` — draw ONE designed image: author an HTML composition per the canvas-design method, rasterize with `render.sh` (browser-screenshot) — exact on-image text, CJK-safe (ENSFont recipe included). Depends on canvas-design (pre-flight stops if missing) + browser-screenshot (render.sh hard-fails); same IMAGE_OK contract as genimage-img2/genimage-nb. Slash `/genimage-canvas`.
