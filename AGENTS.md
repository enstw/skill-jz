...
- `browser-cdp/SKILL.md` — the one browser foundation: SEE a rendered page (gstack daemon route when installed, else hardened `scripts/shot.sh` + `cdp-shot.mjs` Bun CDP pipeline), DRIVE a page over CDP (copy-in `templates/find-browser.mjs` discovery: `BROWSER_BIN` → provisioned wrapper → desktop installs → playwright shell; `templates/cdp-client.mjs`: `launch` → `evalJs`/`send`/`close`), and PROVISION a user-space Chromium where none exists (`scripts/provision.sh`: no root, lib extraction via `apt download` + `dpkg -x`, sandbox probe, self-contained `BROWSER_BIN` wrapper at `~/.cache/headless-chromium/chrome` — the cache path is an on-disk contract, keep it). Absorbed the former browser-screenshot and headless-chromium skills. `scripts/` must stay shipped — `genimage-canvas/gen-image.sh` calls `shot.sh` path-to-path. Preempts `--headless --screenshot` one-liners (149+ writes nothing), `npm i puppeteer/playwright`, fresh CDP clients, and `sudo apt install chromium`. Description-triggered + `/browser-cdp`.
...
- `flush/`, `sync/`, `init-agents/`, `recommend/`, `self-evaluate/`, `robust-web-fetch/`, `pdf-to-markdown/`, `browser-cdp/`, `browser-e2e/`, `genimage-img2/`, `genimage-nb/`, `genimage-canvas/`, `yt2sub/`, `repo-publish/`, ... — one folder per skill.
