...
- `bookworm-e2e/SKILL.md` — run/author the e2e suites of the bookworm repo (enstw/bookworm): suite matrix, dead-server offline runbook, authoring conventions. First project-specific skill in the collection — promoted here so every machine's sessions share it; the project's `CLAUDE.md` points at it. Slash `/bookworm-e2e`.
