...
- `browser-screenshot/SKILL.md` — headless screenshot or rendered-DOM dump of any URL or local HTML/SVG file. Routes through the shared gstack browse daemon (`$B`) when installed; falls back to the bundled `scripts/shot.sh` (hardened CDP pipeline: reused profile + GNU `timeout` hard-kill + `Singleton*` lock cleanup) when gstack is absent. `scripts/` must stay shipped either way — `genimage-canvas/gen-image.sh` calls `shot.sh` path-to-path. Description-triggered.
...
- `genimage-canvas/SKILL.md` — draw ONE designed image: `gen-image.sh` invokes Claude Code's stock canvas-design skill to author an HTML composition, then rasterizes with browser-screenshot — exact on-image text, CJK-safe. Depends on authenticated `claude` + stock canvas-design + browser-screenshot; same `gen-image.sh` name/args/IMAGE_OK contract as genimage-img2/genimage-nb (all three honor `GENIMAGE_TIMEOUT`). Slash `/genimage-canvas`.
- `yt2sub/SKILL.md` — download and transcribe YouTube videos or local audio files (yt-dlp + faster-whisper). Slash `/yt2sub`.
- `browser-e2e/SKILL.md` — give any web app a real-browser e2e suite with zero test dependencies: plain node scripts driving headless Chromium over CDP. Bundles copy-in templates (`e2e-browser.mjs` discovery, `e2e-cdp.mjs` client, example suite), the suite taxonomy (pure-node / self-contained / server-backed), the verdict contract, and the dead-server rule for service-worker offline testing (emulation never reaches SWs). Preempts both `npm i playwright` and pasting a fresh CDP client. Description-triggered + `/browser-e2e`.
- `bookworm-e2e/SKILL.md` — project overlay for the e2e suites of the bookworm repo (enstw/bookworm): suite matrix, the two-stage dead-server offline runbook, bookworm conventions. Defers all method to `browser-e2e`. First project-specific skill in the collection — promoted here so every machine's sessions share it; the project's `CLAUDE.md` points at it. Slash `/bookworm-e2e`.
- `repo-publish/SKILL.md` — publish a local folder to GitHub as a complete presence: pre-publish audit (secrets-in-history, PII, redistribution rights, license) feeding an explicit public-or-private decision (private-first when in doubt), README authored from real contents, `gh repo create` + description/topics, banner + 1280×640 social card via the img primitives, and the manual social-preview upload step (no GitHub API). Pre-condition: `gh` authenticated. Description-triggered + `/repo-publish`.
...
- `flush/`, `sync/`, `init-agents/`, `recommend/`, `self-evaluate/`, `robust-web-fetch/`, `pdf-to-markdown/`, `browser-screenshot/`, `genimage-img2/`, `genimage-nb/`, `genimage-canvas/`, `yt2sub/`, `repo-publish/`, ... — one folder per skill.
