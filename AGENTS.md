...
- `genimage-canvas/SKILL.md` — draw ONE designed image: `render.sh` invokes Claude Code's stock canvas-design skill to author an HTML composition, then rasterizes with browser-screenshot — exact on-image text, CJK-safe. Depends on authenticated `claude` + stock canvas-design + browser-screenshot; same IMAGE_OK contract as genimage-img2/genimage-nb. Slash `/genimage-canvas`.
