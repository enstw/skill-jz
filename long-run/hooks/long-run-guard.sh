...
# Refusal = {"decision":"block","reason":…} on stdout, which makes the agent continue.
#
...
# Loop brake: after LONG_RUN_MAX_IDLE (default 3) refusals in a row with no new tool
# call in between, the stop is allowed — the agent is not working, only talking.
...
# Final assistant text: prefer the hook field, else the last text block in the transcript.
msg=$(printf '%s' "$input" | jq -r '.last_assistant_message // empty')
if [ -z "$msg" ] && [ -r "$tx" ]; then
  msg=$(tail -n 400 "$tx" | jq -c 'select(.type=="assistant") | .message.content[]? | select(.type=="text") | .text' 2>/dev/null | tail -n 1 | jq -r . 2>/dev/null)
fi
...
# No valid stop line: refuse, unless the agent has stopped making progress.
tools=0
[ -r "$tx" ] && tools=$(grep -c "\"type\":\"tool_use\"" "$tx" 2>/dev/null) || tools=${tools:-0}
last_tools=-1; idle=0
[ -r "$state" ] && read -r last_tools idle < "$state"
